...
if (isset($_POST) && !empty($_POST)) {
    ...
    if ($idFilm != $film['id_film'] || !isset($quantity) || empty($quantity) ||  $quantity > $stock ) {
     ...
     foreach ($_SESSION['panier'] as $key => $value) {
         ...
         if ($value['id_film'] == $idFilm ) {
               ...
               $_SESSION['panier'][$key]['quantite'] += $quantity;
     ...
     <!-- le paramètre vider=1 pour indiquer qu'il faut vider le panier. -->
          <?php
                  if (empty($_SESSION['panier'])) {
                   $info= alert('Votre panier est  vide ', 'warning');
                  }    else{  //si non j'affiche le tableau avec les film stocké dans le panier 
     ...
     <a href="?vider=unset" class="btn align-self-end mb-5">Vider le panier</a>
               ...
               ?>
                    <tr>
                         ...
                         <?=$filmDansPanier['quantite']?>
                               <!-- Afficher la quantité actuelle -->

                         </td>
                         <td  class="text-center border-top border-dark-subtle" ><?=$filmDansPanier['price'] * $filmDansPanier['quantite']?>€</td>
